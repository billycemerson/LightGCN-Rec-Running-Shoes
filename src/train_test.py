...
def train(model, data, optimizer):
    ...
    z_users, z_places = model.forward()  # encode
    loss = model.BPRLoss(model.decode_all(z_users, z_places),
...
def test(model, data_val_test):
    ...
    for data in data_val_test:
        z_users, z_places = model.forward()  # encode train
        link_logits_u2a = model.decode(z1=z_users, z2=z_places, pos_edge_index=data.edge_index_u2a,
